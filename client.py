...
class client(object):
    ...
    NINJA_API_GET_DEVICES         = "/v2/devices"
    NINJA_API_GET_ALERTS          = "/v2/alerts"
    NINJA_API_GET_ALERTS_SINCE    = "/v1/alerts/since"
	
    NINJA_API_GET_INSTALLER    = "/v2/organization"
    NINJA_API_GET_ORGS    = "/v2/organizations"
    ...
    def get_installer(self, org_id, loc_id):
        resp = self.api_get_request(self.NINJA_API_GET_INSTALLER + "/" + str(org_id) + "/location/" + str(loc_id) + "/installer/WINDOWS_MSI")
        resp_json = resp.json()
        installer = resp_json["url"]
        r = requests.get(installer, allow_redirects=True)
        open('ninja.msi', 'wb').write(r.content)
        if not resp.status_code == self.STATUS_OK:
            return False
        return installer

    def get_orgs(self):
        resp = self.api_get_request(self.NINJA_API_GET_ORGS)
        if not resp.status_code == self.STATUS_OK:
            return False
        return resp.json()
